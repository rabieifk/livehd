...
void Inou_firrtl::CheckRefForComp(Lnast &ln, const Lnast_nid &ref_node, firrtl::FirrtlPB_Module_UserModule *umod) {
  ...
  if ((name.substr(0, 2) == "__") || (name.substr(0, 5) == "false") || (name.substr(0, 4) == "true")) {
    ...
    // $ = input
    if (io_map.find(name.substr(1).data()) != io_map.end())
      return;
    ...
    // # = register
    if (reg_wire_map.find(name.substr(1).data()) != reg_wire_map.end())
      return;
    ...
    if (reg_wire_map.find(new_name) != reg_wire_map.end())
      return;
    ...
    // otherwise = wire
    if (reg_wire_map.find(name.data()) != reg_wire_map.end())
      return;
