...
class JsonComposer
{
  ...
  public:
    JsonComposer(ostream& target_medium) : target(target_medium) { indent = ""; current_key = NULL;}
    void Write(const JsonElement* el_list) const;
};

template <class T> void jsn::Batch<T>::WriteEach(IterT start, IterT end, const JsonComposer* jcm) const {
  if (start == end) 
    return;
  JsonElement model[] = { {}, {} };
  for(auto itr=start ; ; ) {
...
template <class T> void jsn::Array<T>::WriteItem(const ItemType* item, JsonElement* model, const JsonComposer* jcm) const {
  model[0] = *item;
  model[0].key = NULL;
  jcm->Write(model);
}

template <class T> void jsn::VectorAsObject<T>::WriteItem(const ItemType* item, JsonElement* model, const JsonComposer* jcm) const {
  model[0][*item] = *item;
  jcm->Write(model);
}
