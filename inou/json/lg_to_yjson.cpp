...
vector<Port*> LGtoYJson::primitive_3inp_ABY {new Port("A", pdInput), new Port("B", pdInput), new Port("C", pdInput), new Port("Y", pdOutput) };
...
Prototype LGtoYJson::primitives[] = {
  ...
  Prototype(Ntype_op::TupAdd,   &LGtoYJson::primitive_3inp_ABY),
...
void LGtoYJson::conncet_cell(Module *module, Cell *cell, Node *node){
  ...
  if (op == Ntype_op::Sum) {
    ...
    for (auto &edge : node->inp_edges()) {
      printf("--: %s \n", edge.driver.debug_name().c_str());
      switch (edge.sink.get_pid()) {
        case 4: cell->add_connection(PIN_WIRE("value")); break;
        case 5: cell->add_connection(PIN_WIRE("field")); break;
        case 0: cell->add_connection(PIN_WIRE("parent")); break;
      }
    }
    cell->add_connection(module->get_wire(node->get_driver_pin().get_wire_name().to_s()));
  } else if (op == Ntype_op::TupGet || op == Ntype_op::AttrSet || op == Ntype_op::AttrGet) {
    node->dump();
...
void LGtoYJson::import_io_ports(Lgraph *lg, yjson::Module *module) 
{
  ...
  if (inp_io_node.out_edges().size() > 0) {
