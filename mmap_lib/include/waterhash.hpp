...
static inline constexpr uint64_t _watermum(const uint64_t A, const uint64_t B) {
  uint64_t r = A * B;
  return r - (r >> 32);
}
...
static inline constexpr uint32_t waterhash(const void *key, uint32_t len, uint64_t seed) {
  ...
  switch (len & 15) {
    ...
    case 9:
      seed = _watermum(_waterr32(p) ^ seed, _waterr32(p + 4) ^ _waterp2) ^ _watermum(seed ^ _waterp4, _waterr08(p + 8) ^ _waterp3);
      break;
    case 10:
      seed = _watermum(_waterr32(p) ^ seed, _waterr32(p + 4) ^ _waterp2) ^ _watermum(seed, _waterr16(p + 8) ^ _waterp3);
      break;
    case 11:
      ...
      break;
    case 12:
      seed = _watermum(_waterr32(p) ^ seed, _waterr32(p + 4) ^ _waterp2) ^ _watermum(seed ^ _waterr32(p + 8), _waterp4);
      break;
    case 13:
      seed = _watermum(_waterr32(p) ^ seed, _waterr32(p + 4) ^ _waterp2)
    ...
    case 15:
      seed = _watermum(_waterr32(p) ^ seed, _waterr32(p + 4) ^ _waterp2)
             ^ _watermum(seed ^ _waterr32(p + 8), (_waterr16(p + 12) << 8 | _waterr08(p + 14)) ^ _waterp4);
      break;
  }
